...
class HMMBlock(tf.keras.layers.Layer):
    ...
    def build(self, input_shape: tuple[int | None, ...]) -> None:
        if self.config.embed is not None:
            self.input_proj = tf.keras.layers.Dense(
                ...
                use_bias=False,
                activation="softmax" if self.config.pre_softmax else "sigmoid",
            )
        ...
        if self.config.d_out is not None:
            self.output_weight = self.add_weight(
                shape=(
                    self.config.heads * (
                        2 if self.config.use_reverse_strand else 1
                    ),
                    15,
                    self.config.d_out,
                ),
    ...
    def call(
        ...
        elif self.config.pre_softmax:
            x = tf.nn.softmax(x, axis=-1)  # type: ignore
        else:
            x = tf.nn.sigmoid(x)
        ...
        if self.config.d_out is not None:
            x = tf.einsum("...hs,hsd->...d", posterior, self.output_weight)
        else:
            x = tf.reshape(posterior, tf.concat((
                tf.shape(posterior)[:2],  # type: ignore
                (tf.shape(posterior)[2]  # type: ignore
                 * tf.shape(posterior)[3], )  # type: ignore
            ), 0))
        return x
    ...
    def compute_output_shape(
        ...
        if self.config.d_out is not None:
            return input_shape[:-1] + (self.config.d_out, )
